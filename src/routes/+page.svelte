<script lang="ts">
	...
	const onDateRangeChange = (dateRange: string) =>
		goto(`/?date-range=${dateRange}&language=${data.selectedLanguage}`);

	const onLanguageChange = (language: string) =>
		goto(`/?date-range=${data.selectedDateRange}&language=${language}`);
...
<h1 class="text-2xl text-mono text-center my-4">{data.title}</h1>
...
<form action="/" method="get" class="grid grid-cols-5 gap-4 max-w-md p-2 m-auto">
	<div class="col-span-2 flex flex-col gap-2">
		...
		>
			{#each ['top', 'day', 'week', 'month'] as range}
				<option value={range} selected={data.selectedDateRange === range}
					>{getDateRangeTitle(range)}</option
				>
			{/each}
		</select>
	</div>
	...
	<div class="col-span-2 flex flex-col gap-2">
		...
		>
			{#each data.languages as { title, slug }}
				<option value={slug} selected={slug === data.selectedLanguage}>{title}</option>
			{/each}
		</select>
	</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
	{#each data.repos as { html_url, full_name, description, stargazers_count, language, diff }, idx}
		<a
			href={html_url}
			...
			<div class="inline-flex gap-4 w-full">
				...
				{#if diff}
					<span class="text-sm text-gray-400 flex-1 text-right"
						>{diff} stars
						{#if data.selectedDateRange === 'day'}
							today
						{:else if data.selectedDateRange === 'week'}
							this week
						{:else if data.selectedDateRange === 'month'}
							this month
						{/if}
					</span>
				{/if}
