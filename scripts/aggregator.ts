...
export const groupByLanguage = (repos: Repo[]) =>
	groupBy(repos, (repo: Repo) => repo.language ?? 'other');
...
export const getTrendingPeriod = (
	...
	return Array.from(Object.entries(groupByFullName(repos)))
		.flatMap(([_, snapshots]): { repo: Repo; starsDiff: number }[] => {
			...
			const diffHours =
				(new Date(latestSnapshot.fetchedAt).getTime() -
					new Date(earliestSnapshot.fetchedAt).getTime()) /
				1000 /
				60 /
				60;
			if (diffHours > hours) {
...
export const getTrending = (repos: Repo[], limit: number) => {
	...
	return {
		...
		week: getTrendingPeriod(repos, [endOfDay(addWeeks(now, -1)), endOfDay(now)], 24 * 7).slice(
			0,
			limit
		),
		month: getTrendingPeriod(repos, [endOfDay(addMonths(now, -1)), endOfDay(now)], 24 * 31).slice(
			0,
			limit
		)
	};
